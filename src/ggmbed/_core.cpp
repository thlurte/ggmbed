...
class Embedder {
public:
  Embedder(const std::string &gguf_path,
  ...
  nb::ndarray<float, nb::numpy, nb::device::cpu>
  encode_tokens(const std::vector<std::vector<int32_t>> &batch_seqs,
    ...
    if (batch_seqs.empty()) {
      ...
      return nb::ndarray<float, nb::numpy, nb::device::cpu>(empty_data, 2,
                                                            shape, owner);
    }
  ...
  nb::ndarray<float, nb::numpy, nb::device::cpu>
  encode(const std::vector<std::string> &texts, size_t max_length,
         bool normalize) {
    if (texts.empty()) {
      float *empty_data = new float[0];
      size_t shape[2] = {0, 0};
...
NB_MODULE(_core, m) {
  ...
  nb::class_<Embedder>(m, "Embedder")
      .def(nb::init<const std::string &, const std::string &, bool, int, int,
           ...
           nb::arg("pooling_type"))
      .def("encode", &Embedder::encode, nb::arg("texts"),
           nb::arg("max_length"), nb::arg("normalize"))
      .def("encode_tokens", &Embedder::encode_tokens,
